...
from utils.utils import hash_arr, run_command, run_command_whole
...
def get_equ_rate_cpp(g1, g2, no_patterns=15000, 
                           simulator='./simulator/simulator_equ', 
                           graph_filename='', 
                           res_filepath='', 
                           fast=False):
    ...
    if graph_filename == '':
        graph_filepath = './tmp/tmp_graph_{}_{}_{}'.format(
            time.strftime("%Y%m%d-%H%M%S"), threading.currentThread().ident, random.randint(0, 1000)
        )
    graph_filepath1 = f"{graph_filepath}_g1.txt" 
    graph_filepath2 = f"{graph_filepath}_g2.txt" 
    ...
    def parse_graph(g, graph_filepath):
        # Parse graph 
        PI_index = g['forward_index'][(g['forward_level'] == 0) & (g['backward_level'] != 0)]
        no_pi = len(PI_index)
        no_nodes = len(g['forward_index'])
    ...
    parse_graph(g1, graph_filepath1)
    parse_graph(g2, graph_filepath2)
    
    # Simulation  
    sim_cmd = '{} {} {}'.format(simulator, graph_filepath1, graph_filepath2)
    stdout, exec_time = run_command_whole(sim_cmd)

    # Remove 
    os.remove(graph_filepath1)
    os.remove(graph_filepath2)
